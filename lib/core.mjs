...
function writeJsonFileSync(file, value) {
  ...
  // (an old drafts.json created 0644 keeps leaking otherwise).
  try {
    fs.chmodSync(file, 0o600);
  } catch {
    // Best effort — the parent dir is already 0700.
  }
}
...
function attachmentChip(attachment, index) {
  ...
  return c(attachment.kind === "image" ? "cyan" : "green", `[${kind} #${attachment.n ?? index + 1} ${name}]`);
}
...
// Matches an attachment token sitting immediately before the cursor.
const ATTACHMENT_TOKEN_BEFORE_CURSOR = /\[(Image|File|Pasted) #(\d+)(?: \+\d+ lines)?\]$/;
...
function tmuxWindowOptionValues(chat) {
  return {
    ...
    // window name or chat id. Clipped tighter than the generic 80 so a long
    // first-prompt title doesn't monopolise the tab strip.
    "@acp_hub_title": compactTmuxText(cleanInline(chat.title || ""), 64) || "New chat",
