...
class AcpPeer {
  ...
  respond(id, result) {
    try {
      ...
      // A request handler may settle just after the adapter exits. That is a
      // normal lifecycle race; reporting it is useful, crashing the daemon is
      // not.
      this.onProtocolError?.(`ACP response dropped: ${error.message}`);
      return false;
    }
  }
  ...
  respondError(id, code, message, data = null) {
    ...
    try {
      ...
      this.onProtocolError?.(`ACP error response dropped: ${writeError.message}`);
      return false;
    }
  }
...
class HubRpcClient {
  ...
  // Fires when the socket to the daemon closes for ANY reason — graceful
  // shutdown, crash, or kill -9. The UI uses it to exit cleanly instead of
  // sitting deaf on a dead connection.
  onClose(handler) {
  ...
  call(method, params = {}) {
    if (this.closed) {
      return Promise.reject(new Error("ACP hub connection is closed"));
    }
    const id = this.nextId;
    this.nextId += 1;
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        if (this.pending.delete(id)) {
          reject(new Error(`ACP hub call timed out: ${method}`));
        }
      }, CALL_TIMEOUT_MS);
      timer.unref?.();
  ...
  // then notify the close handlers exactly once.
  handleClose() {
    if (this.closed) return;
    this.closed = true;
    const error = new Error("ACP hub connection closed");
    for (const pending of this.pending.values()) {
      clearTimeout(pending.timer);
      pending.reject(error);
    }
    ...
    for (const handler of this.closeHandlers) {
      try {
        ...
        // A close handler must never take the client down with it.
      }
    }
  }
...
async function connectHub(timeoutMs = 1000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      socket.destroy();
      reject(new Error("Timed out connecting to ACP hub"));
    }, timeoutMs);
