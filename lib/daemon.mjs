...
class HubDaemon {
  ...
  startPromptTurn(chat, cleanText, prompt) {
    ...
    // Provisional title from the prompt we JUST sent, so the tmux tab reflects
    // the current entry immediately. Adapters send their own title via
    // session_info_update only later in the turn, which left the tab showing
    // the previous prompt the whole time; their update still lands afterwards
    // and overrides this. A manual rename (titlePinned) is never overwritten.
    if (!chat.titlePinned) {
      const provisional = provisionalChatTitle(cleanText);
      if (provisional) chat.title = provisional;
    }
  ...
  renameChat(chatId, title) {
    ...
    if (chat) {
      ...
      // A manual rename wins over the per-prompt provisional title.
      chat.titlePinned = true;
      chat.updatedAt = nowIso();
      this.rememberChat(chat, { makeCurrent: true });
      this.persistState();
      this.broadcast(chat, {
...
// First line of the prompt, inline-cleaned and clipped, in the same shape the
// adapters use for their session titles — so the provisional and the official
// title read alike and the tab doesn't jump styles mid-turn.
function provisionalChatTitle(text) {
  const firstLine = cleanInline(String(text || "").split("\n", 1)[0] || "");
  if (!firstLine) return "";
