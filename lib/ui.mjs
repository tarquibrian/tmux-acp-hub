...
class PopupUi {
  ...
  boxedComposerTitle() {
    ...
    // Quiet = one calm color: the title blends into the rule's accent. Only
    // an INTERACTING agent takes a per-state color (plus auth/error), so a
    // colored title always means "something is happening".
    let main;
    if (token === "permission") {
      ...
      main = c(stateColor, `${frame} ${status}`);
  ...
  inputHint(line) {
    ...
    if (line && line.includes("\n")) return "Enter sends · Ctrl+J or Alt+Enter newline";
    ...
    if (!line) {
      ...
      const modes = modeEntries(this.currentChat?.modes);
      if (modes.length >= 2) {
        return "← menu · Tab mode · Alt+E editor";
      }
      return "← menu · Alt+E editor";
    }
    if (!line.startsWith("/") || line.startsWith("//")) {
      return this.currentChat?.id ? "Enter sends · Alt+Enter newline · Alt+E editor · Ctrl+O menu" : "";
    }
