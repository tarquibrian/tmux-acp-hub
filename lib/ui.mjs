...
class PopupUi {
  ...
  async menuLoop() {
    ...
    for (;;) {
      ...
      if (!selection) {
        // Esc/Ctrl+C: back out of the list — reveal the open chat if there is
        // one, else minimize. The menu process stays alive so the next
        // prefix+M reattaches instantly (killing it raced the reopen against a
        // dying blank pane).
      ...
      try {
        ...
        if (selection.type === "chat") {
          // Switch to the chat's own window instead of rendering it inside the
          // menu window — otherwise a chat that already has a live window shows
          // up in two places. workspace.sh dedups by chat id. Without tmux
          // (no workspace) fall back to rendering it here.
          if (process.env.TMUX) {
