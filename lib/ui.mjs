...
class PopupUi {
  ...
  rawInputTextWidth(
    ...
    if (session?.pinned !== false) {
      // Half-box: only the left indent eats width (no side borders).
      const borderColumns = boxed ? COMPOSER_BOX_SIDE_WIDTH : 0;
      return Math.max(
        8,
  ...
  renderBoxedComposer(session, layout) {
    ...
    const interiorWidth = Math.max(4, safeColumns - COMPOSER_BOX_SIDE_WIDTH);
    ...
    for (let index = 0; index < layout.inputRows; index += 1) {
      ...
      painter
        .to(0, layout.inputRow + index)
        .clearLine()
        .text(`${" ".repeat(COMPOSER_BOX_SIDE_WIDTH)}${interior}`);
    }
