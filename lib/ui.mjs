...
class PopupUi {
  ...
  // Inline picker: /model, /effort, /modes, /access render in the dropdown
  // zone below the input — the composer stays live above the list and the
  // transcript stays on screen, exactly like the command autocomplete.
  ...
  maybeOpenInlinePicker(session) {
    ...
    if (!(this.shouldUseBoxedComposer() && session?.pinned !== false)) return false;
    if (!this.currentChat?.id) return false;
    ...
    this.inlinePicker = {
      title: spec.title,
      hint: `1-${Math.min(spec.items.length, 9)} pick · ↑↓ move · Enter · Esc`,
      items: spec.items,
      index,
      apply: spec.apply,
    };
  ...
  handleInlinePickerKey(session, input, key) {
    ...
    const applyValue = (value) => {
      this.closeInlinePicker(session);
      Promise.resolve(picker.apply(value)).catch((error) =>
        this.logLine(c("red", error.message || String(error))),
      );
    };
    ...
    if (key.name === "return" || key.name === "enter") {
      applyValue(picker.items[picker.index]?.value ?? null);
      return true;
    }
    if (input && /^[1-9]$/.test(input)) {
      const n = Number(input) - 1;
      if (n < picker.items.length) applyValue(picker.items[n].value);
      return true;
    }

    // Any other printable key: the user wants to type — close the list and
    // let the composer handle the key normally.
    this.inlinePicker = null;
    return false;
  }
  ...
  handleRawKeypress(session, input, key, finish) {
    ...
    // Alt+E: draft the prompt in $VISUAL/$EDITOR and reload it on exit —
    // long prompts get a real editor. (Meta chord: free of terminal/emacs
    // collisions, and the vim layer passes non-escape meta keys through.)
  ...
  composerStatusLabelStyled(status) {
    ...
    // Per-state color for the spinner + word only (the rule keeps the accent):
    // each busy state reads at a glance instead of all blending into the
    // accent color. Quiet states stay muted; warn/error keep semantics.
    if (isActiveChatStatus(status)) {
      ...
      const frame = COMPOSER_SPINNER_FRAMES[this.composerSpinnerFrame % COMPOSER_SPINNER_FRAMES.length];
      return c(stateColor, `${frame} ${status}`);
    }
  ...
  inputHint(line) {
    ...
    if (!line.startsWith("/") || line.startsWith("//")) {
      return this.currentChat?.id ? "Enter sends · Alt+Enter newline · ⌥E editor · Ctrl+O menu" : "";
    }
