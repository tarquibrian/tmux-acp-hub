...
class PopupUi {
  ...
  boxedComposerTitle() {
    ...
    // Icon + text carry their own colors (provider accent when quiet, a
    // per-state color while busy); only the rule keeps the border accent.
    let main;
    if (token === "permission") {
      ...
      main = `${coloredProviderIcon(chat)} ${coloredProviderLabel(chat)}`;
    }
